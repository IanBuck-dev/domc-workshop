...
function assessmentSheet(record: AgenticPotentialAssessmentRecord) {
  ...
  const lastRow = 5 + data.length + results.length;
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><dimension ref="A1:J${lastRow}"/><sheetViews><sheetView workbookViewId="0" showGridLines="0"><pane ySplit="6" topLeftCell="A7" activePane="bottomLeft" state="frozen"/><selection pane="bottomLeft" activeCell="A7" sqref="A7"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="18"/><cols>${widths.map((width, i) => `<col min="${i + 1}" max="${i + 1}" width="${width}" customWidth="1"/>`).join("")}</cols><sheetData><row r="1" ht="28">${cell("A1", "Agentische Potenzialbewertung", 2)}</row>${row(2, ["Prozess", record.sourceSnapshot.processName, "Szenario", record.sourceSnapshot.scenario.title, "Assessment-Revision", record.assessmentRevision, "Quellrevision", record.sourceSnapshot.sourceProcessHash, "Erstellt", record.createdAt])}${row(3, ["Bewertet", summary.scored, "Nicht ausreichend belegt", summary.insufficient, "Ausgeschlossen", summary.excluded, "", "", "", ""])}${row(5, headers, 2)}${data.map((values, index) => row(index + 6, values)).join("")}${results.map((values, index) => row(index + 6 + data.length, values)).join("")}</sheetData><autoFilter ref="A5:J${5 + data.length}"/><mergeCells count="1"><mergeCell ref="A1:J1"/></mergeCells><printOptions gridLines="0"/><pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/><pageSetup orientation="landscape" paperSize="9" fitToWidth="1" fitToHeight="0"/></worksheet>`;
}
