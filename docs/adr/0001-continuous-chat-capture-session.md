...
- While a turn runs, the server may translate provider tool events and application reconciliation events into a fixed set of transient Chat Activities. These activities contain only product-safe labels such as reading documents, updating the Process Diagram, or checking open points. Raw reasoning, tool inputs and outputs, file paths, shell output, and compaction details remain server-side.
